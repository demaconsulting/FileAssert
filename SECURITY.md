...
We take the security of FileAssert seriously. If you believe you have found a
...
When using FileAssert, we recommend following these security best practices:

### Input Validation

- Validate command-line arguments and file paths before processing
- Be cautious when processing data from untrusted sources
- Use the latest version of FileAssert to benefit from security updates

### Dependencies

- Keep FileAssert and its dependencies up to date
...
- Run FileAssert with the minimum required permissions
- Avoid running FileAssert as a privileged user unless necessary
...
FileAssert reads and writes files on the local file system. Users should:
...
FileAssert relies on third-party packages. We:
...
Thank you for helping keep FileAssert and its users safe!

[security-advisories]: https://github.com/demaconsulting/FileAssert/security/advisories
