...
- **Pipeline evidence**: `lint.ps1` runs `dotnet sysml2tools lint 'docs/sysml2/**/*.sysml'` against
  the actual FileAssert model and fails the build on any syntax or reference error. The build-docs
  ...
  asserts its content (`WeasyPrint_DesignPdf`). A CI build failure at either step is evidence that
